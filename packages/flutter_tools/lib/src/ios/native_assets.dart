...
Future<NativeAssetsiOSResult?> buildNativeAssetsiOS({
  ...
  final List<Asset> nativeAssets = await NativeAssetsBuildRunner(
    ...
    // TODO(dacoharkes): Pass in iOS SDK.
  ...
  // Installing is done in CMake for iOS, so we can only combine all
  ...
  return NativeAssetsiOSResult(nativeAssetsUri, nativeAssetsPodspecUri);
}

class NativeAssetsiOSResult {
  NativeAssetsiOSResult(this.yamlUri, this.podspecUri);
