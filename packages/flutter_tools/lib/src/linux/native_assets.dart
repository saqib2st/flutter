...
Future<Uri?> buildNativeAssetsLinux({
  ...
  final Map<Asset, Asset> assetTargetLocations = <Asset, Asset>{
    for (final Asset asset in nativeAssets) asset: _targetLocationLinux(asset),
  };
  if (nativeAssets.isNotEmpty) {
    logger.printStatus('Copying native assets to ${buildUri.toFilePath()}.');
    final io.Directory buildDir = io.Directory.fromUri(buildUri);
...
Asset _targetLocationLinux(Asset asset) {
